...
export async function generateMetadata({ params }: VdpPageProps): Promise<Metadata> {
  ...
  const description = vehicle.description?.slice(0, 160) || `View details for this certified ${vehicle.make} ${vehicle.model} electric vehicle at ${dealerName}.`;
  const canonical = getCanonicalUrl(org, `/inventory/${id}`);
      ...
      <div className="min-h-[80vh] flex flex-col items-center justify-center p-6 text-center space-y-8 animate-in fade-in zoom-in duration-500">
        ...
        <div className="space-y-4">
          ...
          <p className="text-xl text-muted-foreground font-medium max-w-md mx-auto">
            The certified electric vehicle you're looking for has either been reserved, sold, or unlisted at {org.name}.
          </p>
        </div>
        <div className="flex flex-col sm:flex-row gap-4 pt-8">
          <Link href={`/${org.slug}/inventory`}>
            <Button size="lg" className="rounded-full px-8 h-14 font-black shadow-lg">
              <ChevronLeft className="mr-2 h-5 w-5" />
              Return to Showroom
            </Button>
          </Link>
          <Link href={`/${org.slug}/request-vehicle`}>
