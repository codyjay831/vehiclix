...
import { registerAction, getOrganizationInfoAction } from "@/actions/auth";
...
export default function RegisterPage() {
  ...
  const [orgInfo, setOrgInfo] = React.useState<{ name: string } | null>(null);

  React.useEffect(() => {
    if (orgId) {
      getOrganizationInfoAction(orgId).then((info) => {
        ...
        <div className="flex flex-col items-center gap-4">
          <Link href="/" className="group flex items-center gap-2">
            ...
            <span className="text-3xl font-black uppercase tracking-tighter italic">
              {orgInfo ? (
                <>
                  {orgInfo.name.split(' ')[0]}
                  {orgInfo.name.split(' ')[1] && <span className="text-primary">{orgInfo.name.split(' ')[1]}</span>}
                </>
              ) : (
        ...
        <Card className="rounded-[3rem] border-2 shadow-[0_20px_40px_-15px_rgba(0,0,0,0.08)] overflow-hidden border-primary/5">
          ...
          <CardContent className="p-10">
            <form onSubmit={handleSubmit} className="space-y-6">
              {orgId && <input type="hidden" name="organizationId" value={orgId} />}
